...
void CDelphiDataMarshal::updateParameters()
{
   ...
   real fZ1Plus  = vctiValence1[0]; // valence 1
   real fZ1Minus = vctiValence1[1];
   real fZ2Plus  = vctiValence2[0]; // valence 2
   real fZ2Minus = vctiValence2[1];

   /*
    * concentration of positive ion
    */
   real fZ1PlusConcentrate = vctfSalt[0]*fZ1Minus;
   real fZ2PlusConcentrate = vctfSalt[1]*fZ2Minus;
   ...
   for (unsigned int i = 0; i < vctstrObject.size(); i=i+2)
   {
      ...
      SGrid<real> gMinCoord = vctapAtomPdb[0].getPose();
      SGrid<real> gMaxCoord = vctapAtomPdb[0].getPose();
      fMaxRadius            = vctapAtomPdb[0].getRadius();
      for (integer j = 1; j < iAtomNum; j++)
      {
         gMinCoord = optMin<real>(gMinCoord,vctapAtomPdb[j].getPose());
         gMaxCoord = optMax<real>(gMaxCoord,vctapAtomPdb[j].getPose());
   ...
   {
      gfMinCoordinate = optMin<real>(gfMinCoordinate,vctapAtomPdb[i].getPose()-vctapAtomPdb[i].getRadius());
      gfMaxCoordinate = optMax<real>(gfMaxCoordinate,vctapAtomPdb[i].getPose()+vctapAtomPdb[i].getRadius());
   }
   ...
   {
      SGrid<real> gVec1 = 2.0*optABS<real>(gfMaxCoordinate-gfBoxCenter);
      real fMaxVal1 = optMax<real>(gVec1);
      
      SGrid<real> gVec2 = 2.0*optABS<real>(gfMinCoordinate-gfBoxCenter);
   ...
   SGrid<real> gLeftBndy  = gfBoxCenter - (1.0/fScale)*(iGrid+1)*0.5;
   SGrid<real> gRightBndy = gfBoxCenter + (1.0/fScale)*(iGrid+1)*0.5;
   ...
   for (integer i = 0; i < iAtomNum; i++)
   {
      ...
      vctgfAtomCoordG.push_back( (vctapAtomPdb[i].getPose()-gfBoxCenter)*fScale+(real)((iGrid+1)/2) );
   }
