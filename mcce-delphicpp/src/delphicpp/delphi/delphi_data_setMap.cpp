...
void CDelphiData::setMap()
{
   ...
   myData["vdrop"]       = pddm->gfPotentialDrop; // SGrid<real>
    ...
    */
   myData["offset"]      = pddm->gfOffCenter; // SGrid<real>

   /*
    * set by ACENTER or ACENT function
    */
   myData["acent"]       = pddm->gfAcent; // SGrid<real>
   ...
   myData["limobject"]   = pddm->vctefExtrema;     // std::vector< SExtrema<real> >
   myData["xn1"]         = pddm->vctgfAtomCoordA;  // std::vector< SGrid<real> >
   myData["xn2"]         = pddm->vctgfAtomCoordG;  // std::vector< SGrid<real> >
   //----------------------- set by IO class ------------------------//
   myData["resnummax"]   = pddm->iResidueNum;
   myData["nmedia"]      = pddm->iMediaNum;   
   myData["medeps"]      = pddm->vctfMediaEps;     // std::vector<real>
   ...
   myData["iatmmed"]     = pddm->vctiAtomMediaNum; // std::vector<integer>
   ...
   myData["iepsmp"]      = pddm->vctgiEpsMap;      // std::vector< SGrid<integer> >
   myData["idebmap"]     = pddm->vctbDielecMap;    // std::vector<bool>
   myData["ibgrd"]       = pddm->vctgiBndyGrid;    // std::vector< SGrid<integer> >
   myData["nqgrd"]       = pddm->iCrg2GridNum;
   myData["chrgv2"]      = pddm->vctgvfCrg2Grid;   // std::vector< SGridValue<real> >
   myData["nqgrdtonqass"]= pddm->vctiCrg2GridMap;  // std::vector<integer>
   myData["atmcrg"]      = pddm->vctgvfAtomCrg;    // std::vector< SGridValue<real> >
   myData["chgpos"]      = pddm->vctgfCrgPoseA;    // std::vector< SGrid<real> >
   myData["scspos"]      = pddm->vctgfSurfCrgA;    // std::vector< SGrid<real> >
   myData["crgatn"]      = pddm->vctiCrgAt;        // std::vector<integer>
   myData["atsurf"]      = pddm->vctiAtSurf;       // std::vector<integer>
   myData["atndx"]       = pddm->vctiAtNdx;        // std::vector<integer>
   myData["scsnor"]      = pddm->vctgfSurfCrgE;    // std::vector< SGrid<real> >
   myData["atmeps"]      = pddm->vctfAtomEps;      // std::vector<real>
   //------------------- set by Solver class ---------------------//
   myData["icount2b"]    = pddm->iDielecBndySum;
   myData["icount1b"]    = pddm->iCrgedGridSum;
   myData["gchrg"]       = pddm->vctfGridCrg;      // std::vector<real>
   myData["gchrgp"]      = pddm->vctgiGridCrgPose; // std::vector< SGrid<integer> >
   myData["ibc"]         = pddm->iCrgBdyGrid;
   myData["cgbp"]        = pddm->vctdgvCrgBndyGrid;// std::vector<SDoubleGridValue>
   myData["phimap"]      = pddm->vctfPhiMap;       // std::vector<real>
   //------------------- set by Energy class ---------------------//
   myData["schrg"]       = pddm->vctfSurfCrgE;     // std::vector<real>
