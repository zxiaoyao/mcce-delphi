...
vector< SGrid<real> > CSite::rforceeps1()
{
   vector< SGrid<real> > afield;

   SGrid<real> sfield = {0.0,0.0,0.0},xyz,vtemp;
   SGrid<int> ixyz;
   real fact,fact1,sc,vvtemp,deltas,sigmap,realds,rmyds,trullo,dist,temp;
   integer p,i,j,cont,iat;
   ...
   for (p = 0; p < iBndyGridNum; p++)
   {
      ...
      xyz.nX = (real)ixyz.nX/fScale+fgBoxCenter.nX;
      xyz.nY = (real)ixyz.nY/fScale+fgBoxCenter.nY;
      xyz.nZ = (real)ixyz.nZ/fScale+fgBoxCenter.nZ;
