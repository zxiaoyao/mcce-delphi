 ...
 *     flag   = integer = 1 ==> calc. only potential
 *                      = 2 ==> calc. only field
...
real CSite::tops(const SGrid<real>& xxo,const SGrid<real>& xxu,const real& crg,const real& eps,const int& flag)
{
   real pot = 0.0;

   real phi[65][65][65];
   real xo[3],axo[3],bxo[3],crgridpos[3][8],faxo[3],fbxo[3],mfo[8],crgrid[8];
   real xu[3],axu[3],bxu[3],trgridpos[3][8],faxu[3],fbxu[3],mfu[8];
   real v[8],e[3],dummy,elf[3][8];
   ...
   for (k = 0; k < 65; k++)
      for (j = 0; j < 65; j++)
         for (i = 0; i < 65; i++)
            ifFileStream.read(reinterpret_cast<char*>(&phi[i][j][k]),sizeof(real));
