...
//---------------------------------------------------------------------
void CDelphiSpace::indverdata(real prbrad)
{
    real jig;
    real cba;
    integer idtemp;
...
//note that cba is the minumum box size. it can be bigger if need be. i.e. can refScale to a larger box if need be.
    ...
    if(space_debug) cout << "### in indverdata: ##" << endl;
    jig=2.0*(1.0/fScale);
...
//2011-05-24 Cycle is introduced to remove GOTO 100 statement
    while(true)
    {
        ...
        lmncb1=optCast <integer,real> (((mxxyz-mnxyz)/cba)+1.);
...
//grdi is just the inverse of cba..,used more...
    grdi=1.0/cba;
    if(space_debug) cout << "lmncb1, grdi: " << lmncb1 << " " << grdi << endl;
}// void indverdata;

//---------------------------------------------------------------------


void CDelphiSpace::indver(integer extot1)
{
...
//2011-05-24 Using operations on coord and int_coord type
//variables defined in module operators_on_coordinates
        iexpos[i]=optCast <integer,real> ( (expos[i]-mnxyz)*grdi );
...
//2011-05-24 Array deallocation is done with DEALLOCATE statement
    ...
    if(iexpos.size() > 0 ) vector < SGrid <integer> > ().swap(iexpos);
