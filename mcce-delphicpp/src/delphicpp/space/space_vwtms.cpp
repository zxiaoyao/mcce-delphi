...
//2011-05-17 Arrays allocated by ordinary F95 allocate statement
    ...
    if(space_debug) cout << "############ start vdwToMs: ##############" << endl;
...
*/
    ...
    iarv=0; // Lin Li
    SGrid <integer> * ibnd = new SGrid <integer> [ibmx];
...
//NB change limits to those of the molecule. set for iepsmp NOT equal to unity
    ...
    if(space_debug) cout << "HERE are problems: "<< endl;
...
//operators_on_coordinates
    ...
    if(space_debug) cout << "Problems up to here: "<< endl;

    iBoundNum=n;
    iBoundNumsurf=nn;
    nn=0;
    if(space_debug) cout << "iboundNum,iBoundNumsurf: " << iBoundNum << " " << iBoundNumsurf << endl;
...
//calculate an identity for this conformation
        ...
        if(space_debug) cout << "rsm: " << rsm << endl;
...
//cout << "### flag0: " << endl;
        ...
        //iab2 = new int[nnn];
        //icume = new int[extot];
        if(space_debug) cout << "nnn: " << nnn << endl;
...
//reentrant points is only known in sclbp, filling reentrant regions due to molecules in objects might fail
                    ...
                    xg=(x1*optCast<real,integer>(ixyz))+x1xyz;
...
//in pointers module and float grdi declared and thus accessible in qlog module
                            ...
                            jxyz=optCast<integer,real>(xxyyzz);
                            jx=jxyz.nX;
                            jy=jxyz.nY;
                            jz=jxyz.nZ;
...
//it might be in the contact region findthe closest atom surface
                            ...
                            it123=optCast<integer,real>((s123-xyzo)*cbai);
...
//projecting it to the atom's acc surface and checking against the acc volumes of nearby atoms
                                ...
                                it123=optCast<integer,real>((u123-xyzo)*cbai);
    ...
    */
    ...
    //fScale bondary grid point positions relative to acc data
        if (isolv&&(irea||logs||lognl||isen||isch))
        {
            ...
            {
                scspos[j]=optCast<real,integer>(ibgrd[j]);
            ...
            atsurf=&atsurf_v[-1];
            //atsurf = new integer [iBoundNum+1];
            ...
            if (!isite && scsnor_v.size() >0 ) vector<SGrid <real> >().swap(scsnor_v); //no need to deallocate for vectors
        }// if
