...
//---------------------------------------------------------------------
void CDelphiSpace::cubedata(real fac, real cbln)
{
...
//2011-05-27 Declarations added due to IMPLICIT NONE
    ...
    if(space_debug) cout << "### in cubedata: fac,cbln: " << fac << " " << cbln << endl;
    ...
    lmncb=optCast<integer,real> ((xyzp-xyzo)/cbln);
    //cout << "xyzp,xyzo,cbln,lmncb: " << xyzp << xyzo << cbln << lmncb << endl;
    lcb=lmncb.nX;
    mcb=lmncb.nY;
    ncb=lmncb.nZ;
    if(space_debug) cout << "lmncb" << lmncb << endl;
...
// cbai in qlog
    ...
    if(space_debug) cout << "dataobject_v.size(): " << dataobject_v.size() << endl;
    for (i=0;i<=iNObject-1;i++){
        if(space_debug) cout << "i,dataobject_v[i]: " << i << " " <<  dataobject_v[i] << endl;
...
//icbn will contain also coord center of fictious atoms
    for(ii=1; ii<=iNObject; ii++)
    {
        ...
        {
            if(space_debug) cout << "### Warning: strtmp.substr(0,4)!= is a && kind!=2 ###" << endl;
            itmp=ii+iNatom;
            ...
            for(iz=0; iz<=ncb; iz++)
            {
                for(iy=0; iy<=mcb; iy++)
                {
                    for(ix=0; ix<=lcb; ix++)
                    {
                        ...
                        xq=((optCast<real,integer>(ixyz)+0.5)*cbln)+xyzo;
