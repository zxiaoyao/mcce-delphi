...
//coi contains circle of intersection data for pairs when objects are involved
    struct ccoi
    {
        SGrid <real> xyz; //vector applied in the center of the sphere and pointing to the center of the coi
        real rad; //coi radius;
        integer is; //number of the intersections between an atom and an object
    };// type ccoi;

void CDelphiSpace::sas()
{

    integer nver=520,nedge=1040;
...
//    edgv=0.;
    ...
    if(space_debug) cout <<"radpmax, fRadPrb " << radpmax << fRadPrb[1] << fRadPrb[2] << endl;

    cbln=2.*(fRMax+radpmax); //cube length
    if(space_debug)  cout << "cbln: " << cbln << endl;
...
// vwtms void
    ...
    if(space_debug) cout << "dim, dim1,(natom+nobject-numbmol): " << dim << " " << dim1 << " " << (iNatom+iNObject-numbmol) << endl;
...
//it finds pairs.......
    for(i=1; i<=iNatom; i++)
    {
        ...
        x123=xn1[i];
        ix123=optCast <integer,real> ((x123-xyzo)*cbai);
...
//2011-06-17 Resizing of arrays keeping old values intact
        if ((npr+limu-liml+1)>nprt)
        {
            ...
            if(pls.size() > 0)
            {
                ...
                //deallocate(plstemp);
                vector < SGrid <integer> >().swap(plstemp);
                */
                pls.resize(nprt+1);
            }
            else
...
//+rm(7)*ver[3][iv] has been removed because it is always zero
            ...
            */
            ic123=optCast <integer,real> ((cf123-xyzo)*cbai);
...
END030:
    ...
    //if(allocated(cbn2_v)) deallocate(cbn2_v);
    //if(allocated(cbal)) deallocate(cbal);
    if(pls.size()>0) vector < SGrid <integer> >().swap(pls);
...
/*
    if (iNObject-numbmol>1) // for object, abandoned
    {
        ...
        xmax=omin+side*(0.5+optCast <real,integer> (optCast <integer,real> (0.999+((xmax-xmin)/side))));
