...
void CDelphiSpace::sclbp() // scspos-> scspos; scsnor-> scsnor; iBoundNum-> iBoundNum
{
    ...
    //SGrid <real> scsnor[iBoundNum], scspos[iBoundNum];
    integer nbra[1001];
...
//2011-05-19 Array allocated by ordinary F95 allocate statement
    ...
    if(space_debug) cout << "### in sclbp: ###" << endl;
...
//find the closest surface atom to the gridpoint
        it=optCast <integer,real> ((xg-xyzo)*cbai);
...
//u should have the same value as previous one
            ...
            it=optCast <integer,real> ((u123-xyzo)*cbai);
            nnbr=0;
...
//module and real grdi declared and thus accessible in qlog module
                ...
                dxyz=xxyyzz-optCast<real,integer>(jxyz);
