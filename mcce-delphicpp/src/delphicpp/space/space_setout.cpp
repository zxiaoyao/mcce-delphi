...
//2011-05-12 Some local variables
    ...
    SGrid <real>  sqtemp[31],rad2aavtemp[31];
    SGrid <real> * sq=sqtemp+15;
...
//allocation before if condition
            if(space_debug) cout << "radmax2: " << radmax2 << " radtest: " << radtest << " lim: " << lim << endl;
...
//operators_on_coordinates
                        ...
                        ddist = dist + float(0.25) + optCast <real,integer> (idist);
    ...
    if(space_debug) cout << ibox <<": " << ioff[ibox] << idist << endl;
    ...
    //variables defined in module operators_on_coordinates
                ismin=optCast <integer,real> (sLimGridUnit[ii]%min-temp-1.);
                ismin=min(ismin,iGrid);
                ismin=max(ismin,1);

                ismax=optCast <integer,real> (sLimGridUnit[ii]%max+temp+1.);
                    ...
                    for(ix=ismin.nX; ix<=ismax.nX; ix++)
                    {
                        for(iy=ismin.nY; iy<=ismax.nY; iy++)
                        {
                            for(iz=ismin.nZ; iz<=ismax.nZ; iz++)
                            {
                                ...
                                xyz2=(optCast <real,integer> (ixyz)-xb)*(float(ixyz)-xb);
                                if(sum(xyz2)<=radp2) bDebMap[ix][iy][iz]=ipore;
                                xyz2.nX=xyz2.nX+0.25;

                                ddist=sum(xyz2)+optCast <real,integer> (ixyz)-xb;
    ...
    //operators_on_coordinates
                                ixyz=int_coord(ix,iy,iz);
                                tmpvect1=optCast <real,integer> (ixyz)-xb;
    ...
    //operators_on_coordinates
                    ...
                    for(ix=ismin.nX; ix<=ismax.nX; ix++)
                    {
                        for(iy=ismin.nY; iy<=ismax.nY; iy++)
                        {
                            ...
                            {
                                ixyz=int_coord(ix,iy,iz);
                                tmpvect1=(optCast <real,integer> (ixyz)-fRMid)/fScale+cOldMid;
    ...
    //dot=(P-B)(..-..)
                                ...
                                ixyz=int_coord(ix,iy,iz);
                                xp=(optCast <real,integer> (ixyz)-fRMid)/fScale + cOldMid;
    ...
    //x-offset
                                xp=optCast <real,integer> (ixyz)-xb;
                                xp.nX=xp.nX+0.5;
                                dot=vectz.dot.xp;
...
//check if sphere sits within limits of box
        ...
        ismin=optCast <integer,real> (xn-radmax2-1.0);
        ismax=optCast <integer,real> (xn+radmax2+1.0);
        itest=ismin;
...
//IT HAS PROBLEMS!!!! Walter (be careful before using also in multidilectric case!!!&&!isitmd
            ...
            fxn=optCast <real,integer> (ixn)-xn;
            rad2av=rad2a-fxn;
...
//2011-05-14 Using operations on coord and int_coord type variables defined in module operators_on_coordinates
                            ddxyz=optCast <real,integer> (ixyz)-xn;
                            ddxyz.nX= ddxyz.nX+0.5;
                            dis2min1=optDot(ddxyz,ddxyz)-rad2;

                            ddxyz=optCast <real,integer> (ixyz)-xn2[iac];
                            ddxyz.nX= ddxyz.nX+0.5;
