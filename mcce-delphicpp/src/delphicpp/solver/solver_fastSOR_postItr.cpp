...
void CDelphiFastSOR::conplt(const vector<real>& array,const string& title,const int& iclr,const int& iscl,const int& imk,
                            const int& iplt,const char symb,const int& ixmin,const int& ixmax,vector<string>& iplot,real& ymin,real& ymax)
{
   integer iyup,iylw,iyran;
   real yminl,ymaxl;
   ...
   if (1 == iscl)
   {
      ...
      for (vector<real>::const_iterator it = array.cbegin(); it != array.cend(); ++it)
      {
         if (0.0 < *it)
         {
            ymin = min(ymin,*it);
      ...
      iyup  = (integer)(1.0 + ymaxl); iylw = (integer)(yminl - 1.0);
      iyran = iyup - iylw;
   }
...
void CDelphiFastSOR::postItr(const vector<real>& rmaxl,const vector<real>& rmsl)
{
   /*
    * remap into phimap
    */
   for (integer iy = 0; iy < (iGrid*iGrid*iGrid-1)/2; iy++)
   {
      integer ix = iy*2;
   ...
   {
      int m,n,nn,ii;
      const real *** phimap = pdc->getKey_constPtr<real>("phimap",iGrid,iGrid,iGrid); // pointer to 3D phimap
      int midg = (iGrid+1)/2;
      for (m = 1; m <= 5; m++)
      {
