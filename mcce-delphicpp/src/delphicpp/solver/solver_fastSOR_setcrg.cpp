/**
 * \return:
 *    integer                  iCrgedGridEven    icount1a         number of charged grid points in odd/even arries, to be used in making qval and iqpos
 *    vector<integer>          prgiCrgPose       iqpos(icount1b)
 *    vector<real>             prgfCrgValA       qval(icount1b)   charge value in electron units assigned to each grid point
 *    vector<real>             prgfCrgValG       gval(icount1b)   charge value of each such charge on the grid
 *
 *    integer                  iCrgedGridSum     icount1b         number of charged grid points in odd/even arries, to be used in making qval and iqpos
 *    vector<real>             prgfGridCrg       gchrg(icount1b)  gchrg is the fractional charge in electron units assigned to each grid point
 *    vector< SGrid<integer> > prgigGridCrgPose  gchrgp(icount1b) gchrgp is the position of each such charge on the grid
 *    integer                  iCrgBndyGridNum   ibc              number of charged boundary grid points
...
{
   //++++++++++ INPUT:
   //const integer& nqgrd = iCrg2GridNum;
   const vector< SGridValue<real> >& chrgv2 = prggvCrg2Grid;
   const vector< SGrid<integer> >& iepsmp = prgigEpsMap;
   const vector<bool>& idebmap = prgbDielecMap;
   const int& idirectalg = iDirectEpsMap;
   const vector<real>& medeps = prgfMediaEps;
   vector<integer>::const_iterator nqgrdtonqass = prgiCrg2GridMap.cbegin();
   ...
   vector<real> phimap(iGrid*iGrid*iGrid,0.0); // phimap(1:igrid,1:igrid,1:igrid) = 0.0

   integer inearest[6],iext,ibgp,cont,itmp,ico;
   real temp;
   vector<real> gchrgd,gchrgtmp;
   SDoubleGridValue fdbGridVal;
   vector<integer> gchrg2;

   //++++++++++ OUTPUT:
   integer& icount1a = iCrgedGridEven;
   integer& icount1b = iCrgedGridSum;
   integer& ibc      = iCrgBndyGridNum;
   vector<integer>& iqpos = prgiCrgPose;
   vector<real>& qval = prgfCrgValA, &gval = prgfCrgValG, &gchrg = prgfGridCrg;
   vector< SGrid<integer> >& gchrgp = prgigGridCrgPose;
   vector<SDoubleGridValue>& cgbp = prgdgvCrgBndyGrid;

   for (vector< SGridValue<real> >::const_iterator it = chrgv2.begin(); it != chrgv2.end(); ++it)
   {
      kb1 = it->nGrid.nX; kb2 = it->nGrid.nY; kb3 = it->nGrid.nZ;
      ...
      for (j = 2; j < iGrid; j++)
      {
         for (i = 2; i < iGrid; i++)
         {
            ...
            if(abs(phimap[m]) > fZero) // phimap(i,j,k).ne.0.
            {
               ...
               if (0 != optSum<integer>(igCoord)%2) icount1a += 1;
            }
         }
      }
   }
   ...
   for (vector< SGrid<integer> >::iterator it = gchrgp.begin(); it != gchrgp.end(); ++it)
   {
      ix = it->nX; iy = it->nY; iz = it->nZ; iw = (iz-1)*iGrid*iGrid+(iy-1)*iGrid+(ix-1);
   ...
   for (vector< SGrid<integer> >::iterator it = gchrgp.begin(); it != gchrgp.end(); ++it)
   {
      if (0 != optSum<integer>(*it)%2) { gchrg2.push_back(i); i++; }
      else                             { gchrg2.push_back(j); j++; }
   }

   //---------- determine denominator at all charged grid points
   if (0 != ibc) ibc = 0; ico = 0; i = 0;

   for (vector< SGrid<integer> >::iterator it = gchrgp.begin(); it != gchrgp.end(); ++it)
   {
      ix = it->nX; iy = it->nY; iz = it->nZ;
      ...
      else
      {
         ...
         iw = (iz-1)*iGrid*iGrid + (iy-1)*iGrid + (ix-1);
         itmp = iepsmp[iw].nX/iEpsDim; temp += medeps[itmp];
         itmp = iepsmp[iw].nY/iEpsDim; temp += medeps[itmp];
         itmp = iepsmp[iw].nZ/iEpsDim; temp += medeps[itmp];
      ...
      if (1 == ibgp || 1 == iext)
      {
         ...
         fdbGridVal.fgCoord.nX = (real)ix; fdbGridVal.fgCoord.nY = (real)iy; fdbGridVal.fgCoord.nZ = (real)iz;
         fdbGridVal.fVal1 = gchrgtmp[i]*f4Pi*fScale; fdbGridVal.fVal2 = gchrg2[i];

         cgbp.push_back(fdbGridVal);
      }

      i++;
   } //----- end of for (vector< SGrid<integer> >::iterator it = gchrgp.begin(); it != gchrgp.end(); ++it)
   ...
   qval.assign(icount1b,0.0); gval.assign(icount1b,0.0);
   ...
   {
      j  = gchrg2[i];
      ix = gchrgp[i].nX; iy = gchrgp[i].nY; iz = gchrgp[i].nZ;
      iw = (iz-1)*iGrid*iGrid+(iy-1)*iGrid+(ix+1);
      iqpos[j-1] = iw/2;
   }
...
#ifdef DEBUG_DELPHI_SOLVER_SETCRG
   {
      ...
      index = 0;
      for (vector<integer>::iterator it = iqpos.begin(); it != iqpos.end(); ++it)
      {
         index += 1;
