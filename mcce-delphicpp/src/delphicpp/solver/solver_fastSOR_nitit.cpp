...
void CDelphiFastSOR::nitit(const real& qfact)
{
   integer ix,iy,iz;
   vector<real> rmsl(nxran,0.0),rmaxl(nxran,0.0);
   bool ichangeom,istop,inewfirst,inew;
   real relparprev,factor,fraction,derprec,der;
   int itr,itnum,ires,icountplus;
   real rmsch,rmxch;
   real conv[3] = {0.0,0.0,0.0};
   string nlstr;
...
#ifdef DEBUG_DELPHI_SOLVER
   {
      ...
      ix = 1;
      for (vector<real>::iterator it = debmap1.begin(); it != debmap1.end(); ++it)
      {
      ...
      ix = 1;
      for (vector<real>::iterator it = debmap2.begin(); it != debmap2.end(); ++it)
      {
   ...
   do
   {
...

      {
         ...
         real fac1 = fraction*fDebFct/(2.0*fIonStrength*fEpsOut);
